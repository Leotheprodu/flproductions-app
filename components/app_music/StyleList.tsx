...
export const StyleList = ({
    ...
    const produccionesEstilos = produccioneFiltradas.filter(
        (estilos, index, array) => {
            return !array
                .slice(0, index)
                .some((p) => p.estilo === estilos.estilo);
        }
    );
    ...
    useEffect(() => {
        if (produccioneFiltradas) {
            setEstilos(
                produccionesEstilos.map((element) => element.estilo).sort()
            );
        }
    }, [produccioneFiltradas]);
    ...
    const handleFilteredList = (e) => {
        ...
        setProduccioneFiltradas(
            produccioneFiltradas.filter((element) =>
                element.estilo.includes(filtro)
            )
        );
        if (estilos.length === 1) {
