...
import { DetalleProducciones, useHandleAppMusic, AppMusic } from "../../components";
import { PropsHead } from '../../components/helpers/HeadMetaInfo';
import Head from 'next/head';

function SongDetail({headInfo, producciones}) {
    ...
    return (
        <>
            ...
            <div className=" contenedor songDetail__appmusic">
                ...
                <DetalleProducciones infoProduccion={produccionActual[0]} tipo_obra_general={0} />
...
export const getServerSideProps = async ({ query }) => {
    ...
    const res = await fetch(`${process.env.NODE_ENV === 'production' ? process.env.NEXT_PUBLIC_PROD_PRODUCCIONES : process.env.NEXT_PUBLIC_DEV_PRODUCCIONES}`);
    const data = await res.json();
    const producciones = data.producciones.filter(element => element.tipo_obra === tipo_obra_general && element.id === parseInt(id));
    ...
    const headInfo = {
        ...
        image: `https://img.youtube.com/vi/${produccion.youtube_id}/mqdefault.jpg` || 'https://flproductionscr.com/build_main/img/header-main.png',
